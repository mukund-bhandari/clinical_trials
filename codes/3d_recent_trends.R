...
ggplot(data, aes(x = Phases, y = Completion_Days)) +
  ...
  stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 2)), vjust = -1.9, hjust= 0.9, color = "black", size = 4) +  # Add mean text
  labs(y = "Days_to_Completion (Days)", x = "") +
  theme_classic() +
  theme(
