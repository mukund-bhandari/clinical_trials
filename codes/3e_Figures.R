...
table(data$Study.Results)

data2 <- data %>%
  rename("Study_Results" = 3)%>%
  group_by(Study_Results)%>%
  summarise(count = n())%>%
  arrange(desc(count))
...
# Make the plot
p2 <- ggplot(data2, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Study_Results)) +
...
# Compute label position
data2$labelPosition <- (data2$ymax + data2$ymin) / 2
# Create a label specifically for the percentages and counts, with count on a new line
data2$percentLabel <- sprintf("%d\n(%.1f%%)", data2$count, data2$percentage)  # format to show count and percentage on new lines

# Make the plot
p2 <- ggplot(data2, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = Study_Results)) +
  geom_rect() +  # creates the sections of the doughnut chart
  # Add the text (percentage and count) within the slices. Position them with proper 'x' and 'y' values.
  geom_text(aes(x = 3.5, y = labelPosition, label = percentLabel), color = "black", size = 5, vjust = 0.5) +
  coord_polar(theta = "y") +  # transforms the bar chart into a doughnut chart
  xlim(c(2, 4)) +  # adjusts the space, creating the doughnut hole
  theme_void() +  # removes axes, background, etc.
